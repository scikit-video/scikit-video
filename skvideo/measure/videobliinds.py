...
def motion_feature_extraction(frames):
    ...
    motion_vectors = blockMotion(frames, method='N3SS', mbSize=mblock, p=np.int64(1.5*mblock))
...
def temporal_dc_variation_feature_extraction(frames):
    ...
    mblock=16
    mbsize=16
    ih = np.int64(frames.shape[1]/mbsize)*mbsize
    iw = np.int64(frames.shape[2]/mbsize)*mbsize
    # step 1: motion vector calculation
...
def NSS_spectral_ratios_feature_extraction(frames):
    ...
    dct_diff5x5 = np.zeros((frames.shape[0]-1, np.int64(frames.shape[1]/mblock), np.int64(frames.shape[2]/mblock),mblock**2), dtype=np.float32)
    ...
    lf_gamma5x5 = freq_bands[:, 1:np.int64((mblock**2-1)/3)+1]
    mf_gamma5x5 = freq_bands[:, np.int64((mblock**2-1)/3)+1:2*np.int64((mblock**2-1)/3)+1]
    hf_gamma5x5 = freq_bands[:, np.int64(2*(mblock**2-1)/3)+1:]
