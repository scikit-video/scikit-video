...
def viideo_score(videoData, blocksize=(18, 18), blockoverlap=(8, 8), filterlength=7):
    ...
    scores = []
    for itr in range(0, np.int64(n_len+1), np.int64(step_size)):
      f1_cum = []
      f2_cum = []
      for itr_param in range(itr, np.int64(np.min((itr+gap+1, n_len)))):
...
def viideo_features(videoData, blocksize=(18, 18), blockoverlap=(8, 8), filterlength=7):
    ...
    Mn = np.int64(np.round((M+blockoverlap[0])/np.float32(blocksize[0])))
    Nn = np.int64(np.round((N+blockoverlap[1])/np.float32(blocksize[1])))

    # compute every 2 frames
    features = np.zeros((np.int64(T/2), Mn, Nn, 28), dtype=np.float32)

    for k in range(np.int64(T/2)):
