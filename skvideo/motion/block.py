...
def _minCost(costs):
    h, w = costs.shape
    mi = costs[np.int64((h-1)/2), np.int64((w-1)/2)]
    dy = np.int64((h-1)/2)
    dx = np.int64((w-1)/2)
    #mi = 65535
    #dy = 0
...
def _DS(imgP, imgI, mbSize, p):
    ...
    vectors = np.zeros((np.int64(h / mbSize), np.int64(w / mbSize), 2))
    ...
    for i in range(0, h - mbSize + 1, mbSize):
        for j in range(0, w - mbSize + 1, mbSize):
            ...
            vectors[np.int64(i / mbSize), np.int64(j / mbSize), :] = [x - j, y - i]
    ...
    for i in range(0, h - mbSize + 1, mbSize):
        for j in range(0, w - mbSize + 1, mbSize):
            ...
            if (j == 0):
                ...
                u = vectors[np.int64(i / mbSize), np.int64(j / mbSize) - 1, 0]
                v = vectors[np.int64(i / mbSize), np.int64(j / mbSize) - 1, 1]
                stepSize = np.int64(np.max((np.abs(u), np.abs(v))))
    ...
    for i in range(0, h - mbSize + 1, mbSize):
        for j in range(0, w - mbSize + 1, mbSize):
            ...
            while (stepSize >= 1):
                ...
                costs[:] = 65537
                stepSize = np.int64(stepSize / 2)

            vectors[np.int64(i / mbSize), np.int64(j / mbSize), :] = [y - i, x - j]
...
def _N3SS(imgP, imgI, mbSize, p):
    ...
    h = np.int64(h/mbSize) * mbSize
    w = np.int64(w/mbSize) * mbSize
    imgP = imgP[:h, :w]
    imgI = imgI[:h, :w]

    vectors = np.zeros((np.int64(h / mbSize), np.int64(w / mbSize), 2), dtype=np.float32)
            ...
            for m in range(-stepSize, stepSize + 1, stepSize):
                for n in range(-stepSize, stepSize + 1, stepSize):
                    ...
                    if ((refBlkVer < 0) or
                       ...
                       (refBlkHor + mbSize > w)):
                            continue
                    costRow = np.int64(m / stepSize) + 1
                    costCol = np.int64(n / stepSize) + 1
        ...
        for j in range(0, w - mbSize + 1, mbSize):
            ...
            for m in range(-2, 3, 2):
                for n in range(-2, 3, 2):
                    ...
                    costRow = np.int64(m/2 + 1)
                    costCol = np.int64(n/2 + 1)
                    if ((costRow == 1) and (costCol == 1)):
                        continue
                    costs[costRow, costCol] = _costMAD(imgP[i:i + mbSize, j:j + mbSize], imgI[refBlkVer:refBlkVer + mbSize, refBlkHor:refBlkHor + mbSize])
                    computations = computations + 1
...
def _ES(imgP, imgI, mbSize, p):
    ...
    for i in range(0, h - mbSize + 1, mbSize):
        for j in range(0, w - mbSize + 1, mbSize):
            ...
            dx, dy, mi = _minCost(costs)  # finds which macroblock in imgI gave us min Cost
            vectors[np.int64(i / mbSize), np.int64(j / mbSize), :] = [dy - p, dx - p]
...
def blockMotion(videodata, method='DS', mbSize=8, p=2, **plugin_args):
    ...
    motionData = np.zeros((numFrames - 1, np.int64(height / mbSize), np.int64(width / mbSize), 2), np.int648)
...
def _subcomp(framedata, motionVect, mbSize):
    ...
    for i in range(0, M - mbSize + 1, mbSize):
        for j in range(0, N - mbSize + 1, mbSize):
            dy = motionVect[np.int64(i / mbSize), np.int64(j / mbSize), 0]
            dx = motionVect[np.int64(i / mbSize), np.int64(j / mbSize), 1]
